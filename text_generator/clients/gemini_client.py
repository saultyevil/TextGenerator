...
from text_generator.clients.abstract_client import TextGenerationAbstractClient
from text_generator.error import GenerationFailureError
from text_generator.models import TextGenerationInput, TextGenerationResponse, VisionImage, VisionVideo
...
class GeminiClient(TextGenerationAbstractClient):
    ...
    def _add_to_contents(self, new_content: dict) -> None:
        ...
        while i < len(self._context["contents"]):
            ...
            if self._content_contains_image_type(content):
                num_images += 1
            if num_images > self.MAX_IMAGES_IN_CONTEXT:
                self._remove_message(i)
            i += 1
    ...
    def init_client(self, model_name: str) -> None:
        ...
        self._base_url = f"{gen_ai_url}/{model_name}:generateContent?key={self.api_key}"
        self._count_tokens_url = f"{gen_ai_url}/{model_name}:countTokens?key={self.api_key}"
        self._context = {
            "system_instruction": {
                "parts": [
                    {
